...
async def upload_receipt_to_S3(user_id: int, file: UploadFile = File(...)):
    ...
    receiptJSON = await openai_process_receipt(user_id=user_id, img_url=image_url, file=file)
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": """
                            return the text in this receipt as properly formatted JSON. Only return store name, date, all items and their associated price and quantity. If date is not visible, say "no date". If any values cannot be found, don't include that item. Only provide a JSON response following this format without deviation:
                            { "store_name": "store name", "date": "the date", "items": [ {"name": "item1 name","price": 0.99,"quantity": 5} ] } 
                            """
                        },
                        {
                            "type": "image_url",
